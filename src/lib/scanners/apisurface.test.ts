...
describe('CloudFront', () => {
  ...
  const summary = (id: string) =>
    `<DistributionSummary><Id>${id}</Id><ARN>arn:aws:cloudfront::1:distribution/${id}</ARN><Status>Deployed</Status><DomainName>${id}.cloudfront.net</DomainName>` +
    ...
    `<Restrictions><GeoRestriction><RestrictionType>none</RestrictionType><Quantity>0</Quantity></GeoRestriction></Restrictions><WebACLId></WebACLId></DistributionSummary>`;
    ...
    fetchMock.mockImplementation((url: string) => {
      if (url.includes('/distribution')) {
        return url.includes('Marker=m2')
          ? xml(`<DistributionList><IsTruncated>false</IsTruncated><Items>${summary('E2')}</Items></DistributionList>`)
          : xml(`<DistributionList><IsTruncated>true</IsTruncated><NextMarker>m2</NextMarker><Items>${summary('E1')}</Items></DistributionList>`);
      }
