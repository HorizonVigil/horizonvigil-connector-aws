...
/** Amazon S3 Glacier — REST-JSON. `-` in the path stands for "the caller's own account", the standard Glacier convention for account-scoped calls. */
export async function scanGlacier(ctx: ScannerContext): Promise<ScannedResource[]> {
  ...
  const res = await client.fetch(`https://glacier.${ctx.region}.amazonaws.com/-/vaults`, { method: 'GET' });
